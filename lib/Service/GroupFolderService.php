...
class GroupFolderService
{
    ...
    private function createSubFolders(string $assoName): void
    {
        try {
            ...
            foreach (['1.1. Préfecture', '1.2. Statuts', '1.3. Fiches objectifs'] as $sub) {
            ...
            foreach (['2.1. RIB + contacts conseiller', '2.2. Relevés de comptes mensuels', '2.3. Notes de frais'] as $sub) {
            ...
            foreach (['3.1. Comptes-rendus mensuels', '3.2. Plan de Gestion', '3.3. Bilan mi-parcours', '3.4. Rapport final', '3.5. Vidéo finale'] as $sub) {
    ...
    private function applyRulesForMapping($rm, $mapping, string $assoName, string $role): void
    {
        ...
        if ($rootNode->nodeExists('officiel')) {
            ...
            if ($officiel->nodeExists('4. Autres')) {
                $this->setRule($rm, $mapping, $officiel->get('4. Autres')->getId(), $writePerms);
            }
            ...
            if ($officiel->nodeExists('2. Comptes')) {
                $comptes = $officiel->get('2. Comptes');
            ...
            if ($officiel->nodeExists('3. Rendus')) {
                $rendus = $officiel->get('3. Rendus');
